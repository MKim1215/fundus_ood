...
def plot_auroc(_pos, _neg, fname=None):
    pos = np.array(_pos[:]).reshape((-1, 1))
    neg = np.array(_neg[:]).reshape((-1, 1))
    examples = np.squeeze(np.vstack((pos, neg)))
    labels = np.zeros(len(examples), dtype=np.int32)
    ...
    plt.savefig(f"/home/minsungkim/FUNDUS_RESEARCH/visualization/ood_detection/{fname}.png")
    plt.clf()


def plot_auroc2(_score, fname=None):
    score = np.array(_score[:]).reshape((-1, 1))
    labels = np.ones(len(score), dtype=np.int32)

    fpr, tpr, _ = sk.roc_curve(labels, score)
    plt.plot(fpr, tpr)
    plt.ylabel('True Positive Rate')
    plt.xlabel('False Positive Rate')
    plt.savefig(f"/home/minsungkim/FUNDUS_RESEARCH/visualization/ood_detection/{fname}.png")
...
def plot_aupr(_pos, _neg, fname=None):
    pos = np.array(_pos[:]).reshape((-1, 1))
    neg = np.array(_neg[:]).reshape((-1, 1))
    examples = np.squeeze(np.vstack((pos, neg)))
    labels = np.zeros(len(examples), dtype=np.int32)
    ...
    aupr = sk.average_precision_score(labels, examples)
    display = sk.PrecisionRecallDisplay(
        recall=recall,
        precision=precision,
